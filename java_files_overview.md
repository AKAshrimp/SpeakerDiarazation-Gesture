...
| 檔案 | 主要職責 |
| --- | --- |
| `app/src/main/java/hk/edu/hkmu/speakerdiarazationdemo/MainActivity.java` | App 的錄音與即時轉寫主流程。處理麥克風權限、語言下拉選單、錄音按鈕，以及把 `TranscriptManager` 發出的最終與 Partial 轉寫內容渲染到畫面上（含已註冊說話者配色）。 |
| `app/src/main/java/hk/edu/hkmu/speakerdiarazationdemo/SettingsActivity.java` | 設定頁。提供字體大小調整、已註冊說話者清單（播放／重錄／刪除）、以及歷史轉寫檔案 (`files/transcripts/*.txt`) 的瀏覽與刪除。 |
...
| `app/src/main/java/hk/edu/hkmu/speakerdiarazationdemo/ConfigManager.java` | 單例，從 `res/raw/config.json` 載入 `AppConfig`。 |
