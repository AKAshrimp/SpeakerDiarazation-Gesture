# SpeakerDiarazationDemo Reading Guide
...
## 1. 從設定開始
1. 看 `app/src/main/res/raw/config.json`：掌握 API key、語言、region、延遲、說話者設定等參數。
...
1. **`SpeechmaticsClient.java`**  
   - `generateTempToken()`：POST 到 `https://mp.speechmatics.com/v1/api_keys?type=rt`，body 帶 `region`（預設 `usa`）。  
...
## 4. UI 與資料管理
- **`SettingsActivity` + `activity_settings.xml`**：調整語言、字體大小、查看轉寫檔案與已註冊說話者。
